...
def lambda_handler(event, context):
	...
	include_fargate_for_savings_plans = os.environ.get('INCLUDE_FARGATE_FOR_SAVINGS_PLANS', 'Disabled') == 'Enabled'
	include_lambda_for_savings_plans = os.environ.get('INCLUDE_LAMBDA_FOR_SAVINGS_PLANS', 'Disabled') == 'Enabled'
	today = datetime.datetime.today()
	# first day of the previous month
	inclusive_start_billing_period = (today - datetime.timedelta(today.day)).replace(day=1)
