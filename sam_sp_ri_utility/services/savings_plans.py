...
def create_savings_plans_custom_line_items(inclusive_start_billing_period, exclusive_end_billing_period,
	...
	inclusive_start_billing_period_string = inclusive_start_billing_period.strftime('%Y-%m')
	exclusive_end_billing_period_string = exclusive_end_billing_period.strftime('%Y-%m')
	custom_line_items = []
	...
	for account in eligible_linked_accounts:
		...
		ec2_normalized_hours_for_account = ec2_normalized_instance_hours_by_linked_account.get(account_id, Decimal(0.0))
		...
		if include_fargate_for_savings_plans:
			total_eligible_compute_running_hours += fargate_vcpu_hours_by_linked_account.get(account_id, Decimal(0.0))
			normalized_hours_by_account[account_id] += fargate_vcpu_hours_by_linked_account.get(account_id,
																								Decimal(0.0))
		if include_lambda_for_savings_plans:
			total_eligible_compute_running_hours += lambda_gb_hours_by_linked_account.get(account_id, Decimal(0.0))
			normalized_hours_by_account[account_id] += lambda_gb_hours_by_linked_account.get(account_id, Decimal(0.0))
	for savings_plan in savings_plans_details:
		for account in eligible_linked_accounts:
			savings_plan_guid = savings_plan.get('SavingsPlanArn').split('/')[-1]
			savings_plan_account_id = savings_plan.get('Attributes').get('AccountId')
			...
			if savings_plan_account_id not in [billing_group_account['AccountId'] for billing_group_account in
				...
				account_benefit = Decimal(savings_plan.get('Savings').get('NetSavings')) * running_hour_percentage
				...
				if account_benefit != 0:
					...
					if not is_dry_run:
						billing_conductor_client.create_custom_line_item(**new_custom_line_item)
